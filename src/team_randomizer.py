...
heroes = [
    ...
    "Phoenix", "Blade", "Angela", "Daredevil", "Gambit", "Rogue", "Deadpool"
]
...
role_colors = {"Strategist": "#b388ff", "Duelist": "#ff80ab", "Vanguard": "#8c9eff", "Flexible": "#ff6b35"}
...
class HeroSlot(QFrame):
    ...
    def set_hero(self, hero):
        ...
        # Handle Deadpool's random role assignment
        if hero == "Deadpool":
            import random
            roles = ["Duelist", "Strategist", "Vanguard"]
            random_role = random.choice(roles)
            self.role.setText(random_role)
            self.role.setStyleSheet(f"color: {role_colors[random_role]}; font-weight: bold;")
        else:
...
class TeamRandomizerPage(QWidget):
    ...
    def get_role(self, hero):
        # Handle Deadpool's flexible role
        if hero == "Deadpool":
            import random
            return random.choice(["Duelist", "Strategist", "Vanguard"])
    ...
    def generate_heroes(self):
        if self.cb_222.isChecked():
            ...
            for role in role_map:
                # Skip Flexible role for 2-2-2 format, handle Deadpool separately
                if role == "Flexible":
                    continue
            ...
            # Add Deadpool to random team if available
            if "Deadpool" in heroes:
                if random.choice([True, False]):
                    team_a.append("Deadpool")
                else:
                    team_b.append("Deadpool")
    ...
    def get_available_heroes(self, used_heroes, clicked_slot):
        ...
        if self.cb_222.isChecked():
            ...
            for h in team_used:
                ...
                if role and role != "Flexible":
                    counts[role] += 1
            
            avail = []
            for role, pool in role_map.items():
                if role == "Flexible":
                    # Handle Deadpool specially - can fit any role that needs filling
                    if "Deadpool" not in global_used:
                        for needed_role in ["Duelist", "Strategist", "Vanguard"]:
                            if counts[needed_role] < 2:
                                avail.append("Deadpool")
                                break
                    continue
